...
describe('Course e2e test', () => {
  ...
  it('should load Courses', async () => {
    ...
    expect(await courseComponentsPage.getTitle()).to.eq('glCoopcycleApp.course.home.title');
  ...
  it('should load create Course page', async () => {
    ...
    expect(await courseUpdatePage.getPageTitle()).to.eq('glCoopcycleApp.course.home.createOrEditLabel');
  ...
  it('should create and save Courses', async () => {
    ...
    await promise.all([
      ...
      courseUpdatePage.deliveryStateSelectLastOption(),
      courseUpdatePage.setPrepTimeInput('5'),
      courseUpdatePage.setDeliveryTimeInput('5'),
      courseUpdatePage.setDeliveryDetailsInput('deliveryDetails'),
      courseUpdatePage.cartSelectLastOption(),
      courseUpdatePage.restaurantSelectLastOption(),
      courseUpdatePage.delivererSelectLastOption(),
      courseUpdatePage.clientSelectLastOption()
    ]);

    expect(await courseUpdatePage.getPrepTimeInput()).to.eq('5', 'Expected prepTime value to be equals to 5');
    expect(await courseUpdatePage.getDeliveryTimeInput()).to.eq('5', 'Expected deliveryTime value to be equals to 5');
    expect(await courseUpdatePage.getDeliveryDetailsInput()).to.eq(
      'deliveryDetails',
      'Expected DeliveryDetails value to be equals to deliveryDetails'
    );
  ...
  it('should delete last Course', async () => {
    ...
    expect(await courseDeleteDialog.getDialogTitle()).to.eq('glCoopcycleApp.course.delete.question');
