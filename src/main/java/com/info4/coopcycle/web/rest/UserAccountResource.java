...
/**
 * REST controller for managing {@link com.info4.coopcycle.domain.UserAccount}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class UserAccountResource {
    ...
    public ResponseEntity<List<UserAccount>> getAllUserAccounts(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of UserAccounts");
        Page<UserAccount> page;
        if (eagerload) {
            page = userAccountRepository.findAllWithEagerRelationships(pageable);
        } else {
